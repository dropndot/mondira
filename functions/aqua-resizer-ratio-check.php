...
if ( !function_exists( 'aq_resize_get_pixelratio' ) ){
	function aq_resize_get_pixelratio(){
		if( isset($_COOKIE["pixel_ratio"]) ){
			...
			?>
<script>writeCookie();function writeCookie(){the_cookie=document.cookie;if(the_cookie){if(window.devicePixelRatio>=2){the_cookie="pixel_ratio="+window.devicePixelRatio+";"+the_cookie;document.cookie=the_cookie;location='<?php $_SERVER['PHP_SELF'] ?>'}}}</script>
			<?php
		}
	}
}
